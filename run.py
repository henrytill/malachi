...
def get_python(use_venv: bool) -> str:
    if use_venv:
        ...
        if not python.exists():
            logger.error(
                "Virtual environment not found. Run './run.py create-env' first"
            )
            sys.exit(1)
        return str(python)
...
def generate_version(git_ref: Optional[str] = None):
    ...
    init_file.write_text(
        f'''"""full-text search indexer for content-addressable systems"""
...
def create_env():
    ...
    if venv_path.exists():
        logger.error(
            f"Environment directory {VENV_DIR} already exists. "
            "Remove it first to create a fresh environment."
        )
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="Malachi task automation")
    parser.add_argument(
        "-e", "--venv", action="store_true", help="Use virtual environment"
    )
    ...
    gen_parser = subparsers.add_parser(
        CommandType.GENERATE.value, help="Generate version file"
    )
    gen_parser.add_argument("-g", "--git-ref", help="Git reference for version")

    subparsers.add_parser(
        CommandType.CREATE_ENV.value, help="Create a new virtual environment"
    )
